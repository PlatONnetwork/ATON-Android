...
public class StringUtils {
	...
	public static String getFileNameFromUrl(String url) {
		...
		return filename;
		/*
		 * int index = url.lastIndexOf('?'); String filename; if (index > 1) { filename = url.substring(url.lastIndexOf('/') + 1, index); } else { filename = url.substring(url.lastIndexOf('/') + 1); }
		 * if (filename == null || "".equals(filename.trim())) {// 如果获取不到文件名称 filename = UUID.randomUUID() + ".apk";// 默认取一个文件名 } return filename;
		 */
	}
	...
	/**
	 * 删除多余的空白行 比如 xxxx/n/n/n/n/nxxx -> xxxx/nxxx
	 */
	...
	/**
	 * 判断字符串是否是有效内容 若单纯由各种 空白符 等组成的字符串，则为无效内容
	 */
