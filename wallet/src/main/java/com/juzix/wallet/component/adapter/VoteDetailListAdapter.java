...
//        viewHolder.setText(R.id.tv_vote_staked_and_unstaked, String.format("%s/%s", NumberParserUtils.getPrettyNumber(BigDecimalUtil.div(item.getLocked(), "1E18"), 4), BigDecimalUtil.div(BigDecimalUtil.mul(item.getValidNum(), item.getPrice()).doubleValue(), 1E18)));
        viewHolder.setText(R.id.tv_vote_staked_and_unstaked, String.format
                ("%s/%s", NumberParserUtils.getPrettyNumber(BigDecimalUtil.div(item.getLocked(), "1E18"), 4),
                        NumberParserUtils.getPrettyNumber(BigDecimalUtil.div(BigDecimalUtil.mul(NumberParserUtils.getPrettyNumber(invalidVoteNum, 0), item.getPrice()).doubleValue(), 1E18),4)));
