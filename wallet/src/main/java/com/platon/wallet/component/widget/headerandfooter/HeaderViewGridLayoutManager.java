package com.platon.wallet.component.widget.headerandfooter;
...
/**
 * Depiction: HeaderViewGridLayoutManager是为适配RecyclerView同时使用{@link HeaderAndFooterAdapter}
 * 和{@link GridLayoutManager}而提供的一个GridLayoutManager子类。它保证了RecyclerView在使用GridLayoutManager
 * 的时候,HeaderView 和 FooterView 依然可以占满一行。所以如果你要同时使用HeaderAndFooterAdapter和GridLayoutManager，
...
public class HeaderViewGridLayoutManager extends GridLayoutManager {
    ...
    public HeaderViewGridLayoutManager(Context context, int spanCount, HeaderAndFooterAdapter adapter) {
    ...
    public HeaderViewGridLayoutManager(Context context, AttributeSet attrs, int defStyleAttr,
                                       int defStyleRes, HeaderAndFooterAdapter adapter) {
    ...
    public HeaderViewGridLayoutManager(Context context, int spanCount, int orientation,
                                       boolean reverseLayout, HeaderAndFooterAdapter adapter) {
